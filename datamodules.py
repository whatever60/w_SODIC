...
class SuperResoNC(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # target: [5, 15, 14]
        target = (self.file['data'][index, height] - self.mean[height]) / self.std[height]
        if self.use_cv2:
...
class Corner2CenterDataset(Dataset):
    def __init__(
        ...
        xs = [(i, j + 1, k) for i in range(h) for k, j in enumerate(range(i, h, seg_num[0])) if k != 0]
        ys = [(i, j + 1, k) for i in range(w) for k, j in enumerate(range(i, w, seg_num[1])) if k != 0]
        if force_square:
            self.anchors = [(x, y) for x, y in product(xs, ys) if x[2] == y[2]]
        else:
    ...
    def __getitem__(self, index):
        ...
        height = np.random.choice(self.height)
        data = torch.from_numpy((self.file['data'][sample_idx, height] - self.mean[height]) / self.std[height])
