...
@torch.no_grad()
def validate_unet():
    checkpoint = './lightning_logs/version_0/checkpoints/epoch=554-step=386834.ckpt'
    ...
    with h5py.File('./data/processed/data.h5') as f:
        ...
        lat0, lat1 = np.sort(f['latitude'][:])[[0, -1]]
        heights = f['altitude'][:]
    ...
    preds = torch.zeros(24, 12, 5, 45, 38)
    xi_idx = [(0, 5), (5, 10), (10, 15)]
    yi_idx = [(0, 5), (4, 9), (8, 13)]
    xt_idx = [(0, 15), (15, 30), (30, 45)]
    yt_idx = [(0, 14), (12, 26), (24, 38)]
    for ((xi0, xi1), (xt0, xt1)), ((yi0, yi1), (yt0, yt1)) in product(zip(xi_idx, xt_idx), zip(yi_idx, yt_idx)):
        for height in torch.arange(data.shape[1]):
            preds[:, height, :, xt0:xt1, yt0: yt1] = model(data[:, height, :, xi0:xi1, yi0:yi1], height.expand(24))
    
    data = preds
    preds = torch.zeros(24, 12, 5, 135, 110)
    ...
    wind_speed = torch.from_numpy(mean)[:, -2:].pow(2).sum(dim=1).sqrt().view(1, 12, 1, 1).expand(24, 12, 150, 140).clone()
    ...
    for station in station_info.itertuples():
        ...
        a = (station.altitude - heights[order]) / (heights[order + 1] - heights[order])
        pred = wind_speed[:, :, order, :] * (1 - a) + wind_speed[:, :, order + 1, :] * a
        submit[station.station_id] = pred[:, :, None]

    lon = np.concatenate([lon0 - np.random.rand(8), np.linspace(lon0, lon1, 135), lon1 + np.sort(np.random.rand(7))])
    lat = np.concatenate([lat0 - np.random.rand(15), np.linspace(lat0, lat1, 110), lat1 + np.random.rand(15)])
    submit['Lon'] = lon.repeat(140).reshape(150, 140)
    submit['Lat'] = lat.repeat(150).reshape(140, 150).T
    io.savemat('./submit_train.mat', submit)
    return preds
